...
<body>
    <header >
        ...
        <div class="sec-div">
            <a href="homepage.php" target="_main" class="home"> Home</a>
            &nbsp; &nbsp; &nbsp;
...
<?php endif; ?>
    <h1 class="r-heading">Owner_Login</h1>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query = "SELECT * FROM owners WHERE owner_email = '$email' AND password = '$password'";
        $result = $conn->query($query);

        if ($result->num_rows > 0) {
            // If login is successful, redirect to homepage.php with the email in GET parameter
            header("Location: owner_homepage.php?email=$email");
            exit();
        } else {
